package com.hysonwayne.nahpiattendance.MyGroup;
...
import com.hysonwayne.nahpiattendance.MyGroup.dialogs.NewSaveChangesDialog;
import com.hysonwayne.nahpiattendance.MyGroup.dialogs.NumberOfTeamsDialog;
import com.hysonwayne.nahpiattendance.MyGroup.dialogs.SavePresetAsDialog;
...
public class TeamRandomizer extends Activity implements OnClickListener, OnTouchListener{
	...
	private ArrayList<com.hysonwayne.nahpiattendance.MyGroup.UpdateObj>updateList;
	...
	public void initializeViews(){
		...
		delName.post(new Runnable() {
			...
			public void run() {
				...
				LayoutParams params = (LayoutParams)presetOptions.getLayoutParams();
	...
	private void addName(){
		...
		if(isPresetLoaded()){ //adds to update List
			updateList.add(new com.hysonwayne.nahpiattendance.MyGroup.UpdateObj("add", name));
		}
	...
	public void onClick(View v) {
		...
		int id = v.getId();
		if (id == R.id.add_action_button) {
			addName();
		} else if (id == R.id.bDelName) {
		...
		} else if (id == R.id.bPresetOptions) {
			openOptionsMenu();
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) { //will call this when you click an menu item
		...
		} else if (itemId == R.id.loadPreset) {
		...
		} else if (itemId == R.id.savePresetAs) {
			openSavePresetAsDialog();
		} else if (itemId == R.id.savePreset) {
			savePreset();
		} else if (itemId == R.id.deletePreset) {
		...
		if(resultCode == RESULT_OK){
			...
			case 1://1 if deleteName
				...
				if(data.getSerializableExtra("updateList") != null){
					updateList = (ArrayList<com.hysonwayne.nahpiattendance.MyGroup.UpdateObj>)data.getSerializableExtra("updateList");
				}
	...
	public void initializeUpdateList(){ //must initialize update if you load a preset
		if(isPresetLoaded() == true){
			updateList = new ArrayList<com.hysonwayne.nahpiattendance.MyGroup.UpdateObj>();
		}
	}
